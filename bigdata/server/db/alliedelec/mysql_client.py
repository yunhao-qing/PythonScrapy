...
class LieCategory(dict):
    ...
    def get_all_cat_id_url(cls):
        '''
        method: get_all_cat_id_url
        return: {'cat_id': 1, 'url': 'adf.com',}
        '''
        global ALLIEDELEC_CONN
        cursor = None
        try:
            cursor = ALLIEDELEC_CONN.cursor(buffered=True, dictionary=True)
            sql = ("select cat_id, url from lie_category "
                    "where parent_id != 0")
            cursor.execute(sql)
...
class LieGoods(dict):
    ...
    @classmethod
    def get_cat_id_goods_id_goods_name_goods_sn_site_url_pdf_url_goods_thumb_goods_desc_provider_name_by_page(cls, 
            goods_id):
        global ALLIEDELEC_CONN
        cursor = None
        try:
            ...
            sql = ("select cat_id, goods_id, goods_name, goods_sn, site_url, "
                    "pdf_url, goods_thumb, goods_desc, provider_name from lie_goods "
...
if __name__ == '__main__':
    print(ALLIEDELEC_CONN)
    ALLIEDELEC_CONN.close()
