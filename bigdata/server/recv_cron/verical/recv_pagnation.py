...
def main():
    global VERICAL_GET_ALL_CATEGORIES
    ...
    channel.queue_declare(queue=VERICAL_STORE_PAGNATION)
    channel.basic_qos(prefetch_count=200)
    channel.basic_consume(callback, queue=VERICAL_STORE_PAGNATION, no_ack=False)
    print('waiting store pagnation task')
