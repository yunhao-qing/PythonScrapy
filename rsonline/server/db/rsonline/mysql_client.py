...
class LieCategory(dict):
    ...
    def get_all_cat_id_url_page_count(cls):
        ...
        try:
            ...
            sql = ("select cat_id, url, page_count from lie_category "
                    "where islast=1 order by cat_id DESC ")
            cursor.execute(sql)
    ...
    @classmethod
    def get_all_cat_id_keywords_url(cls):
        global rsonline_CONN
        cursor = None
        try:
            cursor = rsonline_CONN.cursor(buffered=True, dictionary=True)
            sql = ("select cat_id, keywords,url from lie_category where islast=1 ")
            cursor.execute(sql)
            result = cursor.fetchall()
            cks = []
            for row in result:
...
class LieGoods(dict):
    ...
    @classmethod
    def get_cat_id_goods_id_goods_name_goods_sn_site_url_provider_name_goods_thumb_goods_img_goods_desc_by_page\
                    (cls, goods_id):
        global rsonline_CONN
        cursor = None
        try:
            ...
            sql = ("select cat_id, goods_id, goods_name, goods_sn, site_url, "
                    "provider_name,goods_thumb,goods_img,goods_desc from lie_goods "
