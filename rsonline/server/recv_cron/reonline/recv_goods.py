...
from server.db.rsonline import mysql_client, mongo_client
import time
import threading
import traceback
...
def edit_mongo(v):
    ...
    print('更新mongo')
    rsonline = v
    if rsonline['goods_id'] != -1:
        exist_rsonline = mongo_client.rsonline.getrsonline_by_goods_id(
                v['goods_id'])
        print('正在更新mongo')
        if not exist_rsonline:
            print('这个数据在mongo里不存在, 增加mongo')
            mongo_client.rsonline.addrsonline(rsonline)
        else:
            print('这个数据在mongo里存在，替换mongo')
            goods_id = rsonline['goods_id']
            mongo_client.rsonline.replace_one_by_goods_id(goods_id, rsonline)
    else:
        m_goods_id = mysql_client.LieGoods.get_goods_id_by_goods_sn(v['goods_sn'])
        if m_goods_id is not None:
            v_goods_id = mongo_client.rsonline.getrsonline_by_goods_id(m_goods_id)
            if v_goods_id is not None:
                rsonline['goods_id'] = m_goods_id
                mongo_client.rsonline.replace_one_by_goods_id(m_goods_id, rsonline)
            else:
                rsonline['goods_id'] = m_goods_id
                mongo_client.rsonline.addrsonline(rsonline)
...
def main():
    ...
    channel.queue_declare(queue=rsonline_STORE_GOODS, durable=True)
    channel.basic_qos(prefetch_count=200)
    channel.basic_consume(callback, queue=rsonline_STORE_GOODS, no_ack=False)
